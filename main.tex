...
Anschließend werde ich Herausforderungen bei der Sprachkorrektur und -vorher\-sage aufzeigen (\autoref{sec:gegenwart:herausforderungen}) und mögliche Lösungsansätze für die Zukunft erklären (\autoref{sec:zukunft:mögliche_lösungen}). Dabei wird es sowohl um bereits bestehende, aber noch unausgeprägte als auch theoretisch finalisierte, aber praktisch noch nicht umgesetzte als auch nur theoretisierte Konzepte gehen, deren verschiedene Vor- und Nachteile ich ebenfalls erläutern werde.
...
Bevor in der Sprachverarbeitung ein Algorithmus mit einem Text etwas anfangen kann, muss dieser zuerst in eine Form gebracht werden, die für Maschinen lesbarer und besser verarbeitbar ist als die für uns Menschen gewöhnliche Aneinanderreihung von Zeichen, die \ti{wir} schon automatisch als Gruppen verstehen -- Worte \cite[Abs. 5]{satzzeichen_wie_gehirn_woerter_erfasst}.

Am Beispiel eines Large-Language-Modells nach heutigem Standard lässt es sich leicht veranschaulichen, auch wenn abhängig vom Anwendungsfall die Details abweichen können:

1. Zuerst wird der Eingabetext nach einem speziellen Muster in Einzelteile aufgespalten; das können Wörter, Wortteile oder sogar Buchstaben genauso wie andere Zeichen sein. Diesen Prozess nennt man \ti{tokenization}. \cite[]{}

2. Jedem dieser \ti{tokens} wird nun ein \ti{embedding vector}, ein repräsentativer numerischer Vektor, zugewiesen, der die wahre Bedeutung und den Kontext des \ti{tokens} so nuanciert wie möglich erfassen soll. \cite[Abs. 1, 4]{geeksforgeeks_word_embeddings}

3. Im Falle der Transformer-Modelle, wie alle großen Chatbots sie sind \cite[]{}, werden die Vektoren nun durch eine Reihe sogenannter \ti{attention layers} geschickt, die den gesamten Kontext in einer bisher einzigartigen Art und Weise mit den \tb{embedding vectors} verflechten, der die Relevanz von Wörtern berücksichtigt. \cite[Abs. 1--2]{geeksforgeeks_transformer_attention_mechanism} 

\subsection{Anwendungen}
\label{sec:grundlagen_sprachverarbeitung:anwendungen}
