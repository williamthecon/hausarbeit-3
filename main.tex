...
Algorithmen dieser Kategorie bauen regelmäßig auf einer großen Datenbasis auf, um verlässlich funktionieren zu können \ct{?}{?}. Sie analysieren den gegebenen Datensatz und können die Häufigkeiten, mit denen bestimmte Wortfolgen oder Muster auftreten, erkennen \ct{?}{?}.

Der einfachste Ansatz für ein statistisches Modell ist das \term{N-Gramm}: Hierbei werden die Wahrscheinlichkeiten aller -- im Datensatz enthaltenen -- Wortfolgen von $N$ Worten festgestellt, sodass bei einer vorangegangenen Anzahl von $N-1$ Wörtern das nächste Wort, das im Datensatz am häufigsten nach diesen Wörtern stand, mit einer bestimmten Wahrscheinlichkeit vorhergesagt werden kann. \ct{?}{?}

Dabei kommen folgende Ausprägungen am meisten zur Anwendung:

1. Unigramme (1-Gramme) -- wie häufig kommt ein einziges Wort vor -- \ct{?}{?},

2. Bigramme (2-Gramme) -- wie häufig folgt ein Wort auf ein anderes -- \ct{?}{?} und 

3. Trigramme (3-Gramme) bis 5-Gramme -- wie oft folgt auf eine Kette von Wörtern ein anderes \ct{?}{?}.

Längere Ketten von Wörtern führen zu exponentiell mehr Kombinationsmöglichkeiten \ct{?}{?} und bringen außerdem durch das N-Grammen generell fehlende Kontextverständnis keine nennenswerten Vorteile mehr \ct{?}{?}.

Es gibt viele Abwandlungen, die verschiedene Schwächen von N-Grammen korrigieren sollen, wie beispielsweise bidirektionale oder exponentielle N-Gramme, die Häufigkeiten von sowohl folgenden als auch vorangehenden Wörtern zählen \ct{?}{?} beziehungsweise die zusätzliche Mechanismen in die Wahrscheinlichkeitsrechnung einbauen, die den Kontext mehr berücksichtigen und zu schnelle Rückschlüsse aus den Daten verhindern sollen \ct{?}{?}.

Ein weiterer sehr populärer aber deutlich komplexerer Ansatz ist das \term{neuronale Netz}: Die Parameter einer Reihung von Matrixmultiplikationen werden mit einer idealerweise extrem großen Menge an Daten so lange angepasst, bis aus einer kodierten Wortfolge die Wahrscheinlichkeitsverteilung des darauf folgenden Wortes errechnet werden kann. \ct{?}{?}

Diesem Algorithmus sind in der Theorie keine Grenzen gesetzt, sowohl für die Länge der Wortfolge als auch für die Anzahl der Parameter als auch für die Genauigkeit der Ausgabe \ct{?}{?}. In der Praxis erreichen Modelle allerdings nie perfekte Quoten \ct{?}{?}, da ein gewisses finanzielles Interesse Unternehmen davon abhält, häufig kostenlos und unbegrenzt Benutzern zur Verfügung gestellten\ct{?}{?} Modellen auch nur annähernd genug Rechenkapazität zu geben.
...
Trotzdem sind neuronale Netze älteren Modellen wie N-Grammen weit überlegen, weil sie Kontext in einer Menge und Art berücksichtigen können, die mit N-Grammen unrealistisch zu erreichen ist \ct{?}{?}, selbst wenn theoretisch ein solches System erschaffen werden könnte \ct{?}{?}.
